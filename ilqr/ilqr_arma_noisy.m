function [u_nom,x_nom,cost,train_time,fitcoef] = ilqr_arma_noisy(...
    Model,...
    Task,...
    cur_state,...
    cur_u_traj,...
...
camera_noise = Task.measurement_noise*randn(Task.nm,Task.nSim,horizon+1);
...
for j=1:1:Task.nSim
    ...
    for i=1:1:horizon
        ...
        mexstep('set','ctrl',u_act+noise_switch*1000*Task.training_noise*randn(Model.nu,1),Model.nu);
        mexstep('step',1);
        x2(1:Model.nq,1)=mexstep('get','qpos')';
        x2(Model.nq+1:Model.nsys,1)=mexstep('get','qvel')';
%         x2 = x2+noise_switch*Task.training_noise*randn(Model.nsys,1); % state dependent noise
        sum = sum + sumsqr(x2-x_nom(:,i+1));
%         mexstep('set','qpos',x2(1:Model.nq,1),Model.nq);
%         mexstep('set','qvel',x2(Model.nq+1:Model.nsys,1),Model.nv);
%         mexstep('forward');
        z_camera = Task.Ck*x2+noise_switch*Task.measurement_noise*randn(Task.nm,1);%noise_switch*camera_noise(:,j,i);
...
for i=skip:1:horizon 
    A_r1 = [fitcoef(:,1:Task.nm*Task.qx,i),fitcoef(:,Task.nm*Task.qx+Model.nu+1:end,i)]; % process noise only
    A_aug(:,:,i)=[A_r1;
...
%     B_aug(:,:,i)=[bt;zeros(Task.nm*(Task.qx-1),Model.nu);eye(Model.nu*min(Task.qu-1,1));zeros(Model.nu*(Task.qu-2),Model.nu)];% feedback + kalman
...
elseif strcmp(Model.name, 'acrobot') || strcmp(Model.name, 'cheetah')
...
elseif strcmp(Model.name, 'pendulum') || strcmp(Model.name, 'cartpole')
