...
delta_uact = zeros(Model.nu,Task.nSim,horizon); 
...
for j = 1:1:Task.nSim
    ...
    for r = 1:1:Task.avg_num
        ...
        mexstep('forward');
        for i = 1:1:horizon
            delta_uact(:,j,i) = delta_u(:,j,i)-1*K(:,:,i)*delta_x1(:,i,r);
            mexstep('set','ctrl',u_nom(:,i)+delta_uact(:,j,i),Model.nu);
            mexstep('step',1);
...
for i = 1:1:horizon
    AB = delta_x2_avg(:,:,i+1)*[delta_x1_avg(:,:,i);delta_uact(:,:,i)]'/([delta_x1_avg(:,:,i);delta_uact(:,:,i)]*[delta_x1_avg(:,:,i);delta_uact(:,:,i)]');
    Bid(:,:,i) = AB(:,Model.nsys+1:end);
    Aid(:,:,i) = AB(:,1:Model.nsys)+0*Bid(:,:,i)*K(:,:,i); % 0 for direct form, 1 for closedloop form
end
